...
`projects export` requests actual files from the authenticated server; its help owns the format list. JSON and YAML return the same validated document semantics; YAML is interchange source, not persistence. React ZIP supplies a runnable frontend prototype without a business backend. GLB/glTF supply supported scene geometry and animation. Binary formats require `--output FILE` (or `--out FILE`). PNG/PDF/PPTX, video and 3D exports need a configured cloud/self-host browser renderer; JSON/YAML/HTML/SVG and React ZIP do not. Unsupported encoders and missing bindings return explicit errors. `--revision` binds export to the inspected revision. Video recording is capped at 60 seconds; cloud rendering mixes imported audio/video. PowerPoint preserves editable text/primitives and rasterizes complex nodes.

`render --file design.yaml --format svg` performs offline static rendering with optional `--page` and `--time`, preserves references, and does not fetch private media. Document file extensions select JSON or YAML; stdin requires `--input-format json|yaml`. `projects document get --format yaml`, `projects document put`, `projects create --file`, and `projects import` use the same strict codec. YAML aliases, anchors, merge keys, tags, duplicate/non-string keys, multiple documents, unsafe keys, and non-JSON values are rejected. Comments and formatting do not persist. Offline 3D representations are static; server HTML can include the trusted interactive 3D/timeline viewer, and binary outputs render real WebGL. Remote media must be imported before cloud binary export. Arbitrary HTML/SVG import belongs to the browser parser. Google Slides requires real authorization and supported text/shapes/HTTPS images; complex unsupported nodes fail explicitly.
