...
| `projects import/export`, `render` | JSON/YAML document interchange; authenticated cloud export; offline JSON/YAML/HTML/SVG rendering |
...
Offline `render` supports JSON/YAML/HTML/SVG and preserves asset references without fetching private media. YAML is an explicitly applied source/interchange view: strict parsing rejects aliases, anchors, merge keys, tags, duplicate or non-string keys, multiple documents, unsafe keys, and non-JSON values. It validates through the shared DesignDocument schema; comments and formatting are not persisted, and JSON/revision storage remains canonical. For stdin document input, pass `--input-format json` or `--input-format yaml`. Its 3D representation is static; server HTML export can include the trusted interactive 3D/timeline viewer, while cloud raster export uses real WebGL rendering. Google Slides requires real authorization and supports native text/shapes/HTTPS images, rejecting unsupported complex nodes and private image URLs.
